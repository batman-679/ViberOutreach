...
def update_follow_up_date_callback(lead: dict) -> None:
    """Persist a follow-up date immediately from the date picker."""
    lead_id = lead["id"]
    selected_date = st.session_state.get(f"follow_up_picker_{lead_id}")
...
def complete_task_callback(lead: dict) -> None:
    ...
    lead_id = lead["id"]
    st.session_state[f"follow_up_picker_{lead_id}"] = None
    ...
    for lead in filtered_leads:
        ...
        with st.expander(f"💬 {shop_name} | {city} | {phone}", expanded=False):
            ...
            if not lead.get("is_contacted"):
                ...
                st.date_input(
                    "Set Follow-up Date",
                    key=f"follow_up_picker_{lead_id}",
                    value=st.session_state.get(f"follow_up_picker_{lead_id}"),
    ...
    for streamlit_col, (status, border_color, text_color) in zip(columns, PIPELINE_COLUMNS, strict=False):
        with streamlit_col:
            ...
            for lead in grouped[status]:
                ...
                st.date_input(
                    "Set Follow-up Date",
                    key=f"follow_up_picker_{lead['id']}",
                    value=st.session_state.get(f"follow_up_picker_{lead['id']}"),
                    on_change=update_follow_up_date_callback,
                    args=(lead,),
                )
                pipeline_message = (
