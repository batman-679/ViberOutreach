...
DEFAULT_TEMPLATES_FILE = Path(__file__).with_name("templates.txt")
SETTINGS_FIELDS = [
...
def decode_template_bytes(raw_bytes: bytes) -> str | None:
    """Decode bundled or uploaded template files using the most likely encodings."""
    for encoding in ("utf-8-sig", "utf-8", "cp1251"):
        try:
...
def parse_template_blocks(raw_text: str) -> list[tuple[str, str, str]]:
    """Split a template bundle into (name, body, category) rows."""
    parsed_templates: list[tuple[str, str, str]] = []
    for block in [block.strip() for block in raw_text.split("---") if block.strip()]:
...
def insert_template_rows(template_rows: list[tuple[str, str, str]]) -> int:
    """Insert template rows while skipping duplicates by name and body."""
    imported_count = 0
    connection = get_connection()
    try:
        for name, body, category in template_rows:
            cursor = connection.execute(
                """
...
def seed_default_templates() -> None:
    """Populate SQLite with the bundled templates.txt when the library is empty."""
    if fetch_all_templates_cached():
        return
    if not DEFAULT_TEMPLATES_FILE.exists():
        return

    decoded_text = decode_template_bytes(DEFAULT_TEMPLATES_FILE.read_bytes())
    if decoded_text is None:
        return

    parsed_templates = parse_template_blocks(decoded_text)
    if not parsed_templates:
        return

    if insert_template_rows(parsed_templates):
        clear_cached_app_data()
...
def import_bulk_templates(uploaded_file) -> None:
    ...
    decoded_text = decode_template_bytes(uploaded_file.getvalue())
    if decoded_text is None:
    ...
    imported_count = insert_template_rows(parsed_templates)
    clear_cached_app_data()
    if hasattr(st, "toast"):
